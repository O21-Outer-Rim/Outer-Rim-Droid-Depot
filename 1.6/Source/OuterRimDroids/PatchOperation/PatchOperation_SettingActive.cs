...
namespace OuterRimDroids
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = OuterRimDroidsMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!OuterRimDroidsMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (OuterRimDroidsMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
